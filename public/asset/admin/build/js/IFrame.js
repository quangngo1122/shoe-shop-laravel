...
class IFrame {
    ...
    openTabSidebar(item, autoOpen = this._config.autoShowNewTab) {
        ...
        if (
            (!this._config.allowDuplicates && $(`#${navId}`).length === 0) ||
                ...
                $loadingScreen.show(0, () => {
                    $(`${tabId} iframe`)
                        ...
                        .ready(() => {
                            if (this._config.loadingScreen) {
                                if (
                                    typeof this._config.loadingScreen ===
                                    "number"
                                ) {
                                    setTimeout(() => {
                                        $loadingScreen.fadeOut();
    ...
    removeActiveTab(type, element) {
        if (type == "all") {
            ...
            $(`#${tabId}`).remove();
            if (
                $(SELECTOR_TAB_CONTENT).children().length ==
                $(`${SELECTOR_TAB_EMPTY}, ${SELECTOR_TAB_LOADING}`).length
            ) {
                $(SELECTOR_TAB_EMPTY).show();
            } else {
    ...
    _fixHeight(tabEmpty = false) {
        if ($("body").hasClass(CLASS_NAME_FULLSCREEN_MODE)) {
            ...
            const contentWrapperHeight = parseFloat(
                $(SELECTOR_CONTENT_WRAPPER).css("height"),
            );
            const navbarHeight = $(SELECTOR_TAB_NAV).outerHeight();
            if (tabEmpty == true) {
                setTimeout(() => {
                    $(`${SELECTOR_TAB_EMPTY}, ${SELECTOR_TAB_LOADING}`).height(
                        contentWrapperHeight - navbarHeight,
                    );
                }, 50);
            } else {
                $(SELECTOR_CONTENT_IFRAME).height(
                    contentWrapperHeight - navbarHeight,
                );
            }
        }
