...
class Layout {
    ...
    fixLayoutHeight(extra = null) {
        ...
        const heights = {
            ...
            header:
                $(SELECTOR_HEADER).length > 0 &&
                !$("body").hasClass("layout-navbar-fixed")
                    ? $(SELECTOR_HEADER).outerHeight()
                    : 0,
            footer:
                $(SELECTOR_FOOTER).length > 0
                    ? $(SELECTOR_FOOTER).outerHeight()
                    : 0,
            sidebar:
                $(SELECTOR_SIDEBAR).length > 0
    ...
    _init() {
        ...
        $(SELECTOR_MAIN_SIDEBAR).on("mouseenter mouseleave", () => {
            if ($("body").hasClass(CLASS_NAME_SIDEBAR_COLLAPSED)) {
                this.fixLayoutHeight();
            }
        });

        $(SELECTOR_PUSHMENU_BTN).on(
